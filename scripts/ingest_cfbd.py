...
async def register_source_ids(
    ...
    """Record source identity only; never writes tenant tags or mappings."""
    if not rows:
        return
...
async def flush_batch(
    ...
    async with conn.transaction():
        ...
        await register_source_ids(
            conn, "team",
            [(team_ids[name], name, name) for name in sorted(team_names)],
        )
        await register_source_ids(
            conn, "game",
            [(stable_id("game", game_id), game_id, None) for game_id in games],
        )
        await register_source_ids(
            conn, "play",
            [(stable_id("play", str(row["play_external_id"])), str(row["play_external_id"]), None) for row in rows],
        )
